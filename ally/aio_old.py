...
logger.debug(f"Loading aio version {__version__}")
...
async def talk_http_stream(url: str, method: str = 'PUT') -> Talk[bytes]:
    """Returns a put() function for streaming to an HTTP/S URL."""
    method = method.upper()
    ...
    async def talk(data: bytes | None | Absent = absent) -> bytes | None:
        nonlocal que, response

        # Check for any exceptions from the background task
        try:
            exception = exception_queue.get_nowait()
            if exception:
                logger.error(f"talk: HTTP request failed: {exception}")
                raise exception
        ...
        if data is None:
            if que is not None:
                logger.debug("talk: Closing the request")
                await que.join()
                que = None
            return None
        ...
        if isinstance(data, bytes):
            if que is None:
                raise ValueError("Stream is closed.")
            logger.debug(f"talk: Putting chunk into the queue: {len(data)} bytes")
            await que.put(data)
            return None
        ...
        if data is absent:
            if response is None:
                return None
            logger.debug("talk: Reading the response")
            chunk = await response.content.read(chunk_size)
            if chunk:
                logger.debug(f"talk: Read chunk from response: {len(chunk)} bytes")
                return chunk
            logger.debug("talk: Finished reading the response")
            await response.release()
            response = None
            return None
    ...
    async def data_generator() -> AsyncGenerator[bytes, None]:
        while True:
            if que is None:
                break
            logger.debug("In data_generator, about to read the queue")
            chunk = await que.get()
            if chunk is None:
                logger.debug("data_generator: Read None from queue, breaking")
                break
            logger.debug(f"data_generator: Read chunk from queue: {len(chunk)} bytes")
            yield chunk
    ...
    async def http_request():
        ...
        try:
            async with aiohttp.ClientSession() as session:
                response = await session.request(method, url, data=data_generator())
        except Exception as e:
...
# DEBUG:ally.aio:Loading aio version 0.1.8
# DEBUG:asyncio:Using selector: EpollSelector
# DEBUG:ally.aio:talk: Putting chunk into the queue: 27 bytes
# DEBUG:ally.aio:talk: Closing the request
# DEBUG:ally.aio:In data_generator, about to read the queue
# DEBUG:ally.aio:data_generator: Read chunk from queue: 27 bytes
# DEBUG:ally.aio:In data_generator, about to read the queue
# ERROR:asyncio:Task exception was never retrieved
# future: <Task finished name='Task-6' coro=<ClientRequest.write_bytes() done, defined at /home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/client_reqrep.py:621> exception=ConnectionResetError('Cannot write to closing transport')>
# Traceback (most recent call last):
#   File "/home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/client_reqrep.py", line 637, in write_bytes
#     await self.body.write(writer)
#   File "/home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/payload.py", line 442, in write
#     chunk = await self._iter.__anext__()
#             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/home/sam/allemande/ally/aio.py", line 385, in data_generator
#     chunk = await que.get()
#             ^^^^^^^^^^^^^^^
...
# 
# Traceback (most recent call last):
#   File "/home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/client_reqrep.py", line 658, in write_bytes
#     await writer.write_eof()
#   File "/home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/http_writer.py", line 156, in write_eof
#     self._write(chunk)
#   File "/home/sam/ai/venv/lib/python3.12/site-packages/aiohttp/http_writer.py", line 75, in _write
