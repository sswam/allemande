...
def count(text, model=MODEL_DEFAULT) -> int:
    ...
    obj = client.beta.messages.count_tokens(model=model, messages=[{"role": "user", "content": text}])
...
def response_completion(response):
    ...
    logger.debug(
        "Response: %s", json.dumps({"model": response.model, "stop_reason": response.stop_reason, "completion": completion})
    )
...
def stream(
    message, model=MODEL_DEFAULT, out=sys.stdout, debug=False, token_limit: Optional[int] = None, temperature=DEFAULT_TEMPERATURE
):
