...
demo = gr.Interface(
	fn=process_files,
	inputs=[
		gr.Textbox(lines=1, label="Topic", value=topic_default, placeholder=topic_placeholder),
		gr.Textbox(lines=5, label="Mission", value=mission_default, placeholder=mission_placeholder),
		gr.File(label="Documents", file_count="multiple"),
		gr.Textbox(lines=5, label="URLs", placeholder=urls_placeholder),
		gr.Checkbox(label="Turbo Mode", value=False),
	],
	outputs=[
		gr.Textbox(label="Exit Status"),
		gr.Textbox(label="Messages"),
		gr.Textbox(label="Output Text"),
		gr.File(label="Full Output (Zip)"),
		gr.File(label="Output File (Markdown)"),
		gr.File(label="Output File (HTML)"),
		gr.File(label="Output File (PDF)"),
		gr.File(label="Output File (DOCX)"),
	],
	title=title,
	description=desc,
