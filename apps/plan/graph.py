...
class Editor:
    def __init__(self):
        ...
        #Tk.focus(self.entry)
        # canvas
        self.canvas = Canvas(self.frame, width=640, height=480, background=self.bg)
...
class Arc:
 ...
 #     self.canvasitem.config({"width":self.r, "arrowshape":(self.r*4, self.r*5, self.r)})
...
if len(sys.argv) > 1:
    editor.command_read(sys.argv[1:])
