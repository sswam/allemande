...
default: search-clipped.txt businesses-sorted.txt directories-sorted.txt landmarks-sorted.txt
# other-sorted.txt
...
%.txt: queries-%.txt mission-%.txt search-%.txt
	mkdir -p $*
	while read Q; do \
		echo "## $$Q"; \
		echo; \
		QQ=$${Q#[0-9]*. } ; \
		F="$${Q// /_}.txt" ; \
		cat-sections mission-$*.txt search-$*/$$F | \
			gpt process "$$(< $(PROG_DIR)/$*.prompt)" | \
			tee "$*/$$F" ; \
		echo ; \
		echo ; \
	done < $< | tee $@
