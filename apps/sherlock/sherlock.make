...
default: search-clipped.txt businesses.txt directories.txt
...
queries.txt: mission.txt $(PROG_DIR)/queries.prompt
...
businesses.txt: queries.txt mission.txt search.txt $(PROG_DIR)/businesses.prompt
	mkdir -p businesses
...
directories.txt: queries.txt mission.txt search.txt $(PROG_DIR)/directories.prompt
	mkdir -p directories
	while read Q; do echo "## $$Q"; echo; QQ=$${Q#[0-9]*. } ; F="$${Q// /_}.txt"; cat-sections mission.txt search/$$F | \
