...
int test_mozjs(JSContext* cx) {
    ...
    string code_x = "x = " + to_string(x) + ";";
    JS::SourceText<mozilla::Utf8Unit> source_x;
    if (!source_x.init(cx, code_x.c_str(), code_x.length(), JS::SourceOwnership::Borrowed)) {
        return 1;
    }

    JS::RootedValue x_val(cx);
    if (!JS::Evaluate(cx, options, source_x, &x_val)) {
        return 1;
    }

    string code_y = "y = Math.PI * x;";
    JS::SourceText<mozilla::Utf8Unit> source_y;
    if (!source_y.init(cx, code_y.c_str(), code_y.length(), JS::SourceOwnership::Borrowed)) {
        return 1;
    }

    JS::RootedValue y_val(cx);
    if (!JS::Evaluate(cx, options, source_y, &y_val)) {
        return 1;
    }
