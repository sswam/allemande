...
def fix_html_image(html_image: str, bb_file: str) -> str:
    ...
    for img in doc.xpath("//img"):
        ...
        if not md:
            continue
        if md.width:
            img.set("width0", str(md.width))
        if md.height:
            img.set("height0", str(md.height))
        if md.width and md.height:
            div = gcd(md.width, md.height)
            img.set("style", f"aspect-ratio: {md.width//div}/{md.height//div};")
        if md.alt_text:
            alt = img.get("alt") or ""
            if " ---- " not in alt and not re.match(r"^#\d+", alt):
                alt = md.alt_text + (" ---- " + alt if alt else "")
                img.set("alt", alt)
...
def fix_entities_in_code(html_text):
    """ fix double escaped entities in single-line <code> containers """
    html_text = re.sub(
        r'(?<!<pre class="highlight">)<code>(.*?)</code>',
        lambda m: "<code>" + html.unescape(m.group(1)) + "</code>",
        html_text)
    return html_text
...
#     )
    ...
    # fix double escaped entities in single-line <code> containers
    html_text = fix_entities_in_code(html_text)
