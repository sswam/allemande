...
def _augment_participants_with_humans(history, mentioned_in_mission, room, access_check_cache, agent_name_map, agents):
...
def _setup_mediator(may_use_mediator, config, user, is_human, room, access_check_cache, agent_name_map, everyone_except_user, everyone_except_user_all):
    ...
    if may_use_mediator:
        ...
        mediator = filter_access(mediator, room, access_check_cache, agent_name_map)
...
def _direct_reply(history, user, agents, include_self, include_humans, all_participants, room, access_check_cache, agent_name_map):
    reason = "direct_reply"
...
def _compute_ai_participants(all_participants, all_participants_with_excluded, user, agents, room, access_check_cache, agent_name_map):
    ai_participants = [agent for agent in all_participants if agent_is_ai(agents.get(agent))]
    ai_participants = filter_access(ai_participants, room, access_check_cache, agent_name_map)
    ai_participants_with_excluded = [agent for agent in all_participants_with_excluded if agent_is_ai(agents.get(agent))]
    ai_participants_with_excluded = filter_access(ai_participants_with_excluded, room, access_check_cache, agent_name_map)
...
def _last_ai_speaker(history, user, ai_participants, agents, room, access_check_cache, agent_name_map):
...
def who_should_respond(
    ...
    if not include_humans:
        # the helper already filtered; this keeps behavior identical to original
        pass
    ...
    use_mediator, mediator, is_mediator = _setup_mediator(
        may_use_mediator, config, user, is_human, room, access_check_cache, agent_name_map,
    ...
    if not invoked:
        ai_participants, ai_participants_with_excluded, ai_participants_not_user, ai_participants_with_excluded_not_user = _compute_ai_participants(
            all_participants, all_participants_with_excluded, user, agents, room, access_check_cache, agent_name_map
        )
        default = filter_access(default, room, access_check_cache, agent_name_map)
