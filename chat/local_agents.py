...
async def local_agent(c, agent, _query) -> str:
    ...
    if image_agent:
        ...
        try:
            ...
            if len(pos_neg) == 1:
                pos_neg.append("")
            neg_default = agent.get("neg_default", "[use neg]")
            if pos_neg[1].startswith("-"):
                pos_neg[1] = re.sub(r"^-\s*", "", pos_neg[1])
            elif neg_default not in pos_neg[1]:
                pos_neg[1] = neg_default + " " + pos_neg[1]
            ...
            for v in ["seed", "width", "height", "steps", "cfg_scale", "count", "pag", "hires", "hq", "clip_skip", "denoise", "resize", "cfg_scale_raw", "keep", "denoise2"]:
                if v in unp_vars:
