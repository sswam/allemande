...
def extract_indentation(line: str) -> tuple[str, str]:
...
def align_block(block: list[tuple[tuple, tuple]], block_lines: list[str], spacing: int = 2) -> list[str]:
    ...
    for line in block_lines:
        if block_idx < len(block):
            ...
            indent_original, rest_original = extract_indentation(before_original)

            # Calculate code length from expanded version (for alignment)
            ...
            # Use original indentation + trimmed code content + spacing + rest
            code_content = rest_original.rstrip()

            if is_comment_align:
                result.append(f"{indent_original}{code_content}{' ' * spaces_needed}{after_original}{line_ending}")
            else:
                result.append(f"{indent_original}{code_content}{' ' * spaces_needed}{separator_original} {after_original}{line_ending}")
...
def align_equals(
    ...
    for line in iter(istream.readline, ''):
        process_line(line)
