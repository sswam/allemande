...
find_git_root_for_file() {
    local file=$1
    local resolve_symlinks=$2
...
find_git_root() {
    local file0=${files[0]}
    local resolve_symlinks=0
    local root

    root=$(find_git_root_for_file "$file0" "$resolve_symlinks")
    if [ -n "$root" ]; then
        echo "$root"
        return 0
    fi

    # Try to resolve the first file as a symlink to find the repo
    if [ -n "$file0" ]; then
        resolve_symlinks=1
        ...
        if [ -n "$root" ]; then
            echo "$root"
            echo "$resolve_symlinks" > /tmp/messy.resolve_symlinks.$$
            return 0
        fi
    fi
...
get_staged_or_all_files() {
    local -n result_files=$1

    cd "$git_root" || exit 1
    readarray -t result_files < <(git-mod staged)
...
make_paths_relative_to_git_root() {
    for i in "${!files[@]}"; do
        # shellcheck disable=SC2004
...
handle_list_of_files() {
    ...
    local resolve_symlinks=0
    if [ -e /tmp/messy.resolve_symlinks.$$ ]; then
        resolve_symlinks=$(cat /tmp/messy.resolve_symlinks.$$)
        rm -f /tmp/messy.resolve_symlinks.$$
    fi
    ...
    resolve_missing_files "$resolve_symlinks"
    make_paths_relative_to_git_root
