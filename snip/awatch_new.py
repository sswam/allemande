...
class Watcher:  # pylint: disable=too-many-instance-attributes
    ...
    async def watch_directory(self, path):
        """Add watches for a directory and its subdirectories"""
        if path in self.watched_dirs:
            return
        watched_dirs[path] = awatch(path, watch_filter=self.watch_filter)
        if not self.opts.recursive:
            return
        try:
            for entry in Path(path).iterdir():
                if entry.is_dir() and self.watch_filter(Change.added, str(entry)):
                    await self.watch_directory(str(entry))
    ...
    async def run(self):
        ...
        for path in self.paths:
            if Path(path).is_dir():
                await self.watch_directory(path)
                ...
                for change_type, path in changes:
                    ...
                    if Path(path).is_dir():
                        if change_type == Change.added:
                            await watch_directory(path)
                        elif change_type == Change.deleted:
                            watched_dirs.discard(path)
