...
tokens=`llm count < page.txt | tee page-tokens`
chars_per_token=`calc $chars/$tokens`
chars_per_word=`calc $chars/$words`
tokens_per_word=`calc $tokens/$words`

time llm process -m i "$points_prompt" < page.txt |
perl -e 'undef $/; $_ = <>; s/\A[a-z].*:\n+//is; print' |  # strip any intro heading
tee points.txt

points_prompt_tokens=`printf "%s\n\n" "$points_prompt" | llm count`
summary_prompt_tokens=`printf "%s\n\n" "$summary_prompt" | llm count`

points_chars=`wc -c < points.txt | tee points-chars`
points_words=`wc -w < points.txt | tee points-words`
points_tokens=`llm count < points.txt | tee points-tokens`
...
for model in i c 4; do
	...
	llm count < summary.$model.txt | tee summary-tokens-$model
done
...
for model in i c; do
	...
	llm count < summary-one-step.$model.txt | tee summary-one-step-tokens-$model
done
...
(
	...
	header "Prompt Stats"
	echo "points_prompt_tokens: $points_prompt_tokens"
	echo "summary_prompt_tokens: $summary_prompt_tokens"
	...
	echo "points_tokens: $points_tokens"
...
(
	printf "%s\t" "$name" "$url" "$points_prompt_tokens" "$summary_prompt_tokens" \
		"$chars" "$words" "$tokens" "$points_chars" "$points_words" "$points_tokens" \
